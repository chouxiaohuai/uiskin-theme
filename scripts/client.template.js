...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const COMPOSER_CSS = [
      ...
      // The composer stack ships with z-index:1, which lifts it above the app's
      // z:1000 settings modal — so the chat composer painted on top of the open
      // settings dialog. Drop it to auto so the settings dialog always renders
      // above the composer (still above the messages via DOM order). The glass
      // backdrop is kept.
      '.wSkVaW_composerStack { z-index: auto !important; }',
    ].join('\n')
