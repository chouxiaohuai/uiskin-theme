...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SIDEBAR_CSS = [
      ...
      // bordered HARNESS shows. Everything is percentage-based so it stays
      // correct at ANY sidebar width: the svg fills the brand button
      // (width:100%, never clipped), and the mask cut sits between the k
      // (ends at viewBox x=109.4 of 182) and the system HARNESS (starts at
      // x=111.1) regardless of scale; the badge sits at 61% = right after k.
      '.hHd-Xa_brand svg { display: block; width: 100%; height: auto; -webkit-mask-image: linear-gradient(to right, #000 0 60.45%, transparent 60.9%); mask-image: linear-gradient(to right, #000 0 60.45%, transparent 60.9%); }',
      '.hHd-Xa_brand::after { content: "HARNESS"; position: absolute; left: 61%; top: 50%; transform: translateY(-50%); padding: 1px 6px; border: 1px solid var(--ocean-gold-soft); border-radius: 6px; color: var(--ocean-gold); font-size: 9px; font-weight: 600; letter-spacing: 0.08em; line-height: 14px; white-space: nowrap; }',
